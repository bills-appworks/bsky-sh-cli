...
## 利用方法
提供ファイルに含まれる`install.sh`を実行することにより、インストール（ファイルのコピー、環境変数PATH設定のログインスクリプト編集）を行うことができます。`install.sh`を実行すると、インストール先等の確認が行われます。詳細は[インストールドキュメント](INSTALL-ja.md)を参照してください。
