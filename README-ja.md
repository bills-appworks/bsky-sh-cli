...
このツールは、Unix系環境の多くのシェルスクリプト動作環境において、いくつかの依存ツールによって実行できることを目標としています。
...
- `$HOME/.bsky_sh_cli_<プロファイル名>_rc`ファイル
  - プロファイル別のカスタマイズする設定を記述するファイルです。デフォルトでは作成されません。プロファイル指定実行時にこのファイルが存在する場合は`.bsky_sh_cli_rc`の設定を上書きし、プロファイル固有の設定を行うことができます（一部設定を除く）。
...
## 動作を確認したことがある環境
以下に、このツールの動作を確認したことがある環境を記します。

実装には注意を払っていますが、バージョンアップのたびに全ての環境・機能での確認をしていませんので、動作しない場合がある可能性もあります。

オペレーティングシステムにおいては、特にGNU Core Utilities (coreutils)にあたるツールコマンドの導入状況により、互換性の理由により動作しない場合がある可能性もあります。

ログインシェルはインストールでのPATH設定を対象としています。クライアント実行のランタイムは`/bin/sh`になります。
