...
## 利用方法
提供ファイルに含まれる`install.sh`を実行することにより、インストール（ファイルのコピー、環境変数PATH設定のログインスクリプト編集）を行うことができます。`install.sh`を実行すると、インストール先等の確認が行われます。

スーパーユーザ（sudo実行またはrootユーザ）でインストールすると、全ユーザが利用できるように設定されます。一般ユーザでインストールすると、インストールしたユーザのみで利用できるように設定されます。
